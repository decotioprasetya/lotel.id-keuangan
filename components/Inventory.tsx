...
const Inventory: React.FC<Props> = ({ 
  ...
  const saleValue = batches.filter(b => b.stockType === StockType.FOR_SALE).reduce((sum, b) => sum + (b.currentQty * b.buyPrice), 0);
  ...
  // Aggregate production stock for usage target
        ...
        <div className="lg:col-span-1 space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => setStockType(StockType.FOR_PRODUCTION)} className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold uppercase border transition ${stockType === StockType.FOR_PRODUCTION ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-indigo-600 border-indigo-200'}`}>Produksi</button>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">Nama Barang</label>
                <input type="text" value={productName} onChange={e => setProductName(e.target.value)} placeholder="Nama Produk" className="w-full border rounded-lg p-2" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">Jumlah</label>
                  <input type="number" value={qty} onChange={e => setQty(e.target.value)} placeholder="Qty" className="w-full border rounded-lg p-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">Harga Beli</label>
                  <input type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="Rp" className="w-full border rounded-lg p-2" />
                </div>
              </div>
          ...
          <div className="space-y-3">
            <div className="bg-indigo-900 p-5 rounded-xl text-white shadow-lg">
              <h4 className="text-indigo-300 text-[10px] font-bold uppercase mb-1">Nilai Barang Dagangan</h4>
            ...
            <div className="bg-slate-800 p-5 rounded-xl text-white shadow-lg">
              <h4 className="text-slate-400 text-[10px] font-bold uppercase mb-1">Nilai Bahan Produksi</h4>
        ...
        <div className="lg:col-span-2 space-y-6">
                      ...
                      <tr key={b.id} className={`${b.currentQty === 0 ? 'bg-slate-50 opacity-60' : 'hover:bg-slate-50'} transition group`}>
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1">
                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase w-fit ${b.stockType === StockType.FOR_SALE ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>{b.stockType}</span>
                            {isNext && b.currentQty > 0 && <span className="text-[9px] text-amber-600 font-bold uppercase">FIFO Next</span>}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 min-w-[120px]">
                          ...
                          <div className="w-full bg-slate-100 h-1 rounded-full overflow-hidden">
                            <div className={`h-full bg-indigo-600`} style={{ width: `${percentLeft}%` }} />
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm font-bold text-right text-indigo-600">{formatCurrency(b.currentQty * b.buyPrice)}</td>
                        <td className="px-6 py-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            {b.stockType === StockType.FOR_PRODUCTION && b.currentQty > 0 && (
                              <button onClick={() => setUsageTarget({name: b.productName, available: productionProducts[b.productName]})} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition" title="Pakai Bahan"><Zap size={16} /></button>
                            )}
                            <button onClick={() => onDeleteBatch(b.id)} disabled={b.currentQty < b.initialQty} className={`p-2 rounded-lg transition ${b.currentQty < b.initialQty ? 'text-slate-200 cursor-not-allowed' : 'text-slate-400 hover:text-red-600 hover:bg-red-50'}`}><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    );
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center gap-2">
               ...
               <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Riwayat Pemakaian Bahan</h3>
            </div>
              ...
              <table className="w-full text-left text-xs">
                <thead className="sticky top-0 bg-slate-100 text-slate-500 font-bold">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right">Nilai Biaya</th>
                    <th className="px-6 py-3 text-center">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {productionUsages.length === 0 ? (
                    <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-400 italic">Belum ada pemakaian bahan</td></tr>
                  ) : (
                    [...productionUsages].reverse().map(u => (
                      <tr key={u.id} className="hover:bg-slate-50">
                        <td className="px-6 py-3 whitespace-nowrap">{formatDate(u.date)}</td>
                        <td className="px-6 py-3 font-bold text-slate-800">{u.productName}</td>
                        <td className="px-6 py-3 text-center">{u.qty} unit</td>
                        <td className="px-6 py-3 text-right font-bold text-red-600">{formatCurrency(u.totalCost)}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-slate-800">Pakai Bahan Produksi</h3>
              <button onClick={() => setUsageTarget(null)} className="p-2 hover:bg-slate-100 rounded-full"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                <p className="text-xs text-indigo-600 font-bold uppercase">Bahan</p>
                <p className="text-lg font-black text-indigo-900">{usageTarget.name}</p>
                <p className="text-xs text-indigo-500 mt-1">Stok Tersedia: <span className="font-bold">{usageTarget.available} unit</span></p>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">Tanggal Pemakaian</label>
                <input type="date" value={usageDate} onChange={e => setUsageDate(e.target.value)} className="w-full border rounded-lg p-2" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">Jumlah yang Digunakan</label>
                <input type="number" value={usageQty} onChange={e => setUsageQty(e.target.value)} placeholder="0" className="w-full border rounded-lg p-3 text-lg font-bold" />
              </div>
              <div className="pt-2">
                <button onClick={handleUseSubmit} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 flex items-center justify-center gap-2"><Zap size={18} /> Konfirmasi Pemakaian</button>
                <p className="text-[10px] text-slate-400 text-center mt-3 uppercase font-medium italic">*Nilai bahan akan otomatis dicatat sebagai Biaya di Buku Kas.</p>
              </div>
            </div>
          </div>
        </div>
